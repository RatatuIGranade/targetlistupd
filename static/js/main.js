...
// Enhanced event delegation with browser compatibility
    document.addEventListener('click', function(e) {
        var target = e.target;
        // Check if target supports matches method and is a clickable element
        if (target && target.matches && target.matches('.btn, .category-tab, .filter-btn')) {
            createRipple(e);
        } else if (target && target.classList) {
            // Fallback for browsers without matches
            if (target.classList.contains('btn') || target.classList.contains('category-tab') || target.classList.contains('filter-btn')) {
                createRipple(e);
            }
        }
    });
