...
# Configure the database - optimized for PostgreSQL on Railway
...
if not database_url:
    ...
    database_url = f'sqlite:///{os.path.join(instance_dir, "bedwars_leaderboard.db")}'

app.config['SQLALCHEMY_DATABASE_URI'] = database_url

# Optimized engine options for PostgreSQL
if 'postgresql' in database_url:
    app.config["SQLALCHEMY_ENGINE_OPTIONS"] = {
        ...
        "connect_args": {
            "application_name": "bedwars_leaderboard",
...
with app.app_context():
    ...
    from models import Player, Quest, PlayerQuest, Achievement, PlayerAchievement, CustomTitle, PlayerTitle, GradientTheme, PlayerGradientSetting, SiteTheme, ShopItem, ShopPurchase, PlayerActiveBooster, AdminCustomRole, PlayerAdminRole, Badge, PlayerBadge, ReputationLog, ASCENDData, Clan, ClanMember, Tournament, TournamentParticipant, TargetList, GameMode, ASCENDHistory, InventoryItem

    try:
        # Create tables if they don't exist (production-safe)
        app.logger.info("Ensuring database schema...")
        db.create_all()
