...
class Player(db.Model):
    ...
    def _get_cached_statistics(cls):
        ...
        # Оптимизированный запрос статистики одним запросом
        ...
        result = {
            ...
            'players_with_karma': int(stats.players_with_karma) if stats and stats.players_with_karma else 0,
            'karma_percentage': round((int(stats.players_with_karma) / total_players * 100), 1) if total_players > 0 else 0,
            'top_player': top_player,
            'richest_player': richest_player,
            'most_reputable_player': most_reputable_player,
            'most_karma_player': most_karma_player
        }

        # Кэшируем результат на 10 минут
        from cache import Cache
        Cache.set('player_statistics', result, expire=600)
...
class ShopItem(db.Model):
    ...
    def apply_item_effect(self, player):
        ...
        try:
            ...
            elif self.category in ['booster', 'title', 'custom_role', 'emoji_slot']:
...
class Tournament(db.Model):
    ...
    def type_display(self):
        ...
        type_names = {
            'bedwars_2v8': '🛏️ Bedwars 2v8',
            'bedwars_3v4': '🛏️ Bedwars 3v4',
            'bedwars_3v8': '🛏️ Bedwars 3v8',
            'bedwars_4v4': '🛏️ Bedwars 4v4',
            'bedwars_4v8': '🛏️ Bedwars 4v8',
            'kitpvp_1v1': '⚔️ KitPvP 1v1',
            'skywars': '☁️ SkyWars',
            'duels': '🗡️ Duels',
            'singles': '👤 Одиночный',
            'teams': '👥 Командный',
...
class CustomTitle(db.Model):
    """Custom titles for players"""

    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False, unique=True)
    display_name = db.Column(db.String(200), nullable=False)
    color = db.Column(db.String(7), default='#ffd700', nullable=False)
    glow_color = db.Column(db.String(7), default='#ffd700', nullable=False)
    is_active = db.Column(db.Boolean, default=True, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
Index('idx_player_search', Player.nickname)